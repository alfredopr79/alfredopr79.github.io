...
int** sol;
int** instance;
double PopulationSize=1;//Change if you want to calculate with more sol
...
int main(int args,char** arg){
    ...
    //Create Population
        for (i = 0; i < PopulationSize; i++){
            //CreatePermutations(i);//If you want more solutions uncomment
...
void SimAnnealing(int i){
    ...
    for(T=180; T > 1;){
        for(j = 2; j < n-2;j++){//If LinkNew better than LinkOld change with LinkNew
            ...
            else{//If LinkNew not better then apply Boltzman function
